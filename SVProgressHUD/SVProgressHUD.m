...
static const CGFloat SVProgressHUDVerticalSpacing = 6.0f;
static const CGFloat SVProgressHUDHorizontalSpacing = 12.0f;
static const CGFloat SVProgressHUDLabelSpacing = 18.0f;
...
- (void)updateHUDFrame {
    ...
    CGFloat centerX;
    if(self.statusLabel.text) {
        CGFloat xOffset = MAX(SVProgressHUDHorizontalSpacing, (self.minimumSize.width - contentWidth - SVProgressHUDLabelSpacing - labelWidth) / 2.0f);
        centerX = xOffset + contentHeight / 2.0f;
    } else {
        centerX = CGRectGetMidY(self.hudView.bounds);
    }
    self.indefiniteAnimatedView.center = CGPointMake(centerX , CGRectGetMidY(self.hudView.bounds));
    if(self.progress != SVProgressHUDUndefinedProgress) {
        self.backgroundRingView.center = self.ringView.center = CGPointMake(centerX, CGRectGetMidY(self.hudView.bounds));
    }
    self.imageView.center = CGPointMake(centerX, CGRectGetMidY(self.hudView.bounds));

    // Label
    if(imageUsed || progressUsed) {
        centerX = CGRectGetMaxX(imageUsed ? self.imageView.frame : self.indefiniteAnimatedView.frame) + SVProgressHUDLabelSpacing + labelWidth / 2.0f;
    } else {
        centerX = CGRectGetMidX(self.hudView.bounds);
    }
    self.statusLabel.frame = labelRect;
    self.statusLabel.center = CGPointMake(centerX, CGRectGetMidY(self.hudView.bounds));
...
- (void)positionHUD:(NSNotification*)notification {
    ...
    CGFloat posY = floorf(activeHeight*0.1514f);
...
- (UIImageView*)imageView {
    if(!_imageView) {
        _imageView = [[UIImageView alloc] initWithFrame:CGRectMake(0.0f, 0.0f, 20.0f, 20.0f)];
    }
